...
const ENV_TMDB_API_KEY = import.meta.env.VITE_TMDB_API_KEY?.trim() || '';
...
function getApiKey(): string | null {
  if (typeof window === 'undefined') return ENV_TMDB_API_KEY || null;
  const storedKey = localStorage.getItem(TMDB_API_KEY_STORAGE);
  return storedKey || ENV_TMDB_API_KEY || null;
}
