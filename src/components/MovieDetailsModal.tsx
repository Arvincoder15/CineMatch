    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-6">
          ...
          {/* Tags */}
          <div className="flex flex-wrap gap-2">
            <Badge key="genre" variant="secondary">{movie.genre}</Badge>
            <Badge key="rating" variant="outline">⭐ {movie.rating}</Badge>
            {movie.rating >= 8 && <Badge key="highly-rated" variant="default">Highly Rated</Badge>}
            {movie.year >= 2020 && <Badge key="recent" variant="secondary">Recent</Badge>}
          </div>
        </div>
      </DialogContent>
    </Dialog>
